class OpenaiChatService
  ...
  def handle_ai_phase(selected, force_finish:)
    ...
    @state["turn_count"] += 1
    must_finish = force_finish || @state["turn_count"] >= (mode == "free" ? 5 : 8)
  ...
  def prompt_free_next(freeword)
    ...
    目的: 
    - ユーザーの入力（#{freeword}）をもとに、意図を深掘りするための質問を1つだけ返す。
    - freeword を会話の中心テーマとして扱い、絶対に軸から外れないこと。

    freeword の扱い:
    - freeword のカテゴリ（食事 / 旅行 / 遊び / プレゼント / その他）を推測してよい
    - freeword が表す意図（食べたい / 行きたい / やりたい / やり方/ 買いたい / 癒されたい など）を推測してよい
    - ただし freeword が会話の主軸であることを常に維持すること
    ...
    - 場所を聞く場合は、ある程度の地方が絞れるような質問をすること
    - 直前の質問と似た質問は禁止
    - 回答は毎回少し違う言い回しで自然に
    - 同じ表現を繰り返さないこと
    - titleが決まれば終了することを念頭に置くこと
    ...
    禁止事項:
    - 「これがいい・ちょっとちがう・まあまあ」などの評価を求める質問は禁止
    - 評価系・感想系の質問は禁止
    - freeword を深掘りしない質問は禁止
    - freeword と直接関係しない質問は禁止
    - freeword のテーマから外れる質問（例: 食事・買い物・観光などの一般カテゴリ）は出さない
    - 途中の回答が freeword のテーマを上書きしないようにすること
  ...
  def prompt_free_finish(freeword)
    <<~PROMPT
      ...
      - freeword を中心にした最終提案を1つだけ返す。

      重要ルール:
      - freeword（最初のワード）を必ず主軸にすること
      - 途中の回答は freeword を補足する目的でのみ使うこと
      - freeword と無関係な提案は絶対に避けること
      - 過去に終了したイベント名は title に絶対に含めないこと
      - 提案するtitleが場所・サービス・イベント・スポット等の場合は、現在または今後も体験可能な場所・サービス・イベント・スポットに限定すること
      ...
      - 回答は毎回少し違う言い回しで自然に
      - 同じ表現を繰り返さないこと
      - title には freeword に関連する固有名詞を必ず含めること
      - freeword と無関係な固有名詞は禁止
